      ...
      <div className="grid grid-cols-2 lg:grid-cols-1 gap-3 sm:gap-4 lg:w-72 lg:flex-shrink-0">
          ...
          <div className="flex items-center gap-3">
            <div className="relative w-12 h-12 sm:w-14 sm:h-14">
              <svg viewBox="0 0 60 60" className="w-full h-full -rotate-90">
                ...
                <motion.circle cx="30" cy="30" r="24" fill="none" stroke="hsl(211, 100%, 50%)" strokeWidth="6" strokeLinecap="round"
        ...
        <motion.div custom={1} variants={widgetVariants} initial="hidden" animate="visible" whileHover={{ y: -3 }} className="neo-card p-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <motion.div key={t.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 + i * 0.1 }} className="text-center">
        ...
        <motion.div custom={2} variants={widgetVariants} initial="hidden" animate="visible" whileHover={{ y: -3 }} className="neo-card p-4 col-span-2 lg:col-span-1">
          <div className="flex items-center justify-between mb-3">
            ...
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={onAddGoal} className="text-primary">
              ...
              return (
                <div key={goal.id} className="group">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-1">
                      <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => setEditGoal(goal)} className="p-0.5 rounded hover:bg-accent transition-colors">
                          <Pencil className="w-2.5 h-2.5 text-muted-foreground" />
                        </button>
                        <button onClick={() => handleDeleteGoal(goal.id)} className="p-0.5 rounded hover:bg-destructive/10 transition-colors">
                  ...
                  <div className="w-full h-1.5 bg-muted rounded-full overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${Math.min(progress, 100)}%` }} transition={{ duration: 1, delay: 0.5 }} className="h-full gradient-blue rounded-full" />
                  </div>
        ...
        <motion.div custom={3} variants={widgetVariants} initial="hidden" animate="visible" whileHover={{ y: -3 }} className="neo-card p-4">
          ...
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={onAddGoal} className="w-full gradient-blue text-primary-foreground text-xs font-semibold py-2.5 rounded-xl">
            Crear meta
          </motion.button>
        </motion.div>
